...
export function CodeBlock({ children, language, copyable = true, appearance = 'dark' }: CodeBlockProps) {
  return (
    <div className={cx('overflow-hidden rounded-lg', appearance === 'dark' ? 'dls-inverse bg-ink-900' : 'border border-line bg-canvas')}>
      {(language || copyable) && (
        <div className={cx('flex items-center justify-between px-4 py-[9px]', appearance === 'dark' ? 'border-b border-white/10' : 'border-b border-line')}>
          {language && (
            <span className={cx('font-data text-[13px] uppercase tracking-[0.08em]', appearance === 'dark' ? 'text-ink-400' : 'text-ink-500')}>
              {language}
            </span>
          )}
          {copyable && (
            <button
              type="button"
              className={cx('text-[13px] font-semibold', appearance === 'dark' ? 'text-ink-300' : 'text-brand-600')}
            >
              Copy
            </button>
          )}
      ...
      <pre className={cx('m-0 overflow-auto px-5 py-[18px]', appearance === 'dark' ? 'text-white/85' : 'text-ink-900')}>
        <code className="font-data text-[13px] leading-[1.75]">{children}</code>
      </pre>
    </div>
  );
}
